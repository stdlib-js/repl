...
setNonEnumerableReadOnly( REPL.prototype, 'load', function load( fpath, clbk ) {
	...
	var i;
	if ( this._closed ) {
		throw new Error( format( '0RM12' ) );
	}
	if ( !isString( fpath ) ) {
		throw new TypeError( format( '0RM3R', fpath ) );
	}
	if ( !isFunction( clbk ) ) {
		throw new TypeError( format( '0RM2S', clbk ) );
	}

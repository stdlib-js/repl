...
var Presentation = require( './../../presentation' ); // eslint-disable-line stdlib/no-redeclare
	...
	*/
	function onCommand( text, options ) {
		...
		if ( arguments.length === 0 ) {
			err = new TypeError( format( '0RM0y' ) );
		...
		if ( arguments.length === 1 ) {
			if ( isString( text ) ) {
				...
				err = new TypeError( format( '0RM8I', text ) );
				debug( 'Error: %s', err.message );
				repl._ostream.write( 'Error: '+err.message+'\n' );
				return;
			}
			...
			if ( !isString( text ) ) {
				err = new TypeError( format( '0RM3R', text ) );
				debug( 'Error: %s', err.message );
				repl._ostream.write( 'Error: '+err.message+'\n' );
				return;
			}
			...
			if ( !isPlainObject( options ) ) {
				err = new TypeError( format( '0RM8J', options ) );
				debug( 'Error: %s', err.message );
				repl._ostream.write( 'Error: '+err.message+'\n' );
				return;
			}
		...
		if ( hasOwnProp( opts, 'watch' ) && !isBoolean( opts.watch ) ) {
			err = new TypeError( format( '0RM30', 'watch', opts.watch ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
		if ( t === void 0 && !hasOwnProp( opts, 'load' ) ) {
			err = new TypeError( format( '0RM0z' ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}

	...
	*/
	function onCommand( workspace, variable, value ) {
		...
		if ( !isString( workspace ) ) {
			err = new TypeError( format( '0RM3R', workspace ) );
		...
		if ( !hasOwnProp( repl._workspaces, workspace ) ) {
			err = new Error( format( '0RM8B', workspace ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
		...
		for ( i = 0; i < list.length; i += 2 ) {
			if ( list[ i ] === variable ) {
				...
				if ( typeof desc.get === 'function' || desc.writable === false ) {
					err = new Error( format( '0RM8D', variable ) ); // Note: this mirrors the built-in environment error message
					debug( 'Error: %s', err.message );
					repl._ostream.write( 'Error: '+err.message+'\n' );
					return;
				}

	...
	function onCommand( name, options ) {
		...
		if ( arguments.length === 1 ) {
			if ( isString( name ) ) {
				...
				err = new TypeError( format( '0RM8G', name ) );
			...
			if ( !isString( name ) ) {
				err = new TypeError( format( '0RM3R', name ) );
				debug( 'Error: %s', err.message );
				repl._ostream.write( 'Error: '+err.message+'\n' );
				return;
			}
			...
			if ( !isPlainObject( options ) ) {
				err = new TypeError( format( '0RM2h', name ) );
				debug( 'Error: %s', err.message );
				repl._ostream.write( 'Error: '+err.message+'\n' );
				return;
			}
		...
		if ( hasOwnProp( opts, 'include' ) && !isRegExp( opts.include ) ) {
			err = new TypeError( format( '0RM8P', 'include', opts.include ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
		if ( hasOwnProp( opts, 'exclude' ) && !isRegExp( opts.exclude ) ) {
			err = new TypeError( format( '0RM8P', 'exclude', opts.include ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
		if ( hasOwnProp( opts, 'types' ) && !isArrayLike( opts.types ) ) {
			err = new TypeError( format( '0RM4y', 'types', opts.types ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
		if ( hasOwnProp( opts, 'details' ) && !isBoolean( opts.details ) ) {
			err = new TypeError( format( '0RM30', 'details', opts.details ) );
			debug( 'Error: %s', err.message );
			repl._ostream.write( 'Error: '+err.message+'\n' );
			return;
		}
